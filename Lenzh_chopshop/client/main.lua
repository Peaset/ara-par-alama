...
function ChopVehicle()
    ...
    elseif
        ...
        exports.pNotify:SendNotification({
            text = "" ..math.floor(timerNewChop / 60000).. " Dakika Sonra Tekrar Deneyiniz !",
            type = "error",
            timeout = 1000,
            layout = "centerRight",
            queue = "right",
...
function VehiclePartsRemoval()
    ...
    if ChoppingInProgress == true then
        exports['progressBars']:startUI(Config.DoorOpenFrontLeftTime, "Ön Sol Kapı Açılıyor...")
        Citizen.Wait(Config.DoorOpenFrontLeftTime)
        SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 0, false, false)
    end
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
        exports['progressBars']:startUI(Config.DoorBrokenFrontLeftTime, "Ön Sol Kapı Parçalanıyor...")
        Citizen.Wait(Config.DoorBrokenFrontLeftTime)
        SetVehicleDoorBroken(GetVehiclePedIsIn(ped, false), 0, true)
    end
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
        exports['progressBars']:startUI(Config.DoorOpenFrontRightTime, "Ön Sağ Kapı Açılıyor...")
        Citizen.Wait(Config.DoorOpenFrontRightTime)
        SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 1, false, false)
    end
    Citizen.Wait(1000)
    if ChoppingInProgress == true then
        exports['progressBars']:startUI(Config.DoorBrokenFrontRightTime, "Ön Sağ Kapı Parçalanıyor...")
	...
	if rearLeftDoor ~= -1 then
        if ChoppingInProgress == true then
            exports['progressBars']:startUI(Config.DoorOpenRearLeftTime, "Arka Sol Kapı Açılıyor...")
            Citizen.Wait(Config.DoorOpenRearLeftTime)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 2, false, false)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            exports['progressBars']:startUI(Config.DoorBrokenRearLeftTime, "Arka Sol Kapı Parçalanıyor...")
            Citizen.Wait(Config.DoorBrokenRearLeftTime)
            SetVehicleDoorBroken(GetVehiclePedIsIn(ped, false), 2, true)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            exports['progressBars']:startUI(Config.DoorOpenRearRightTime, "Arka Sağ Kapı Açılıyor...")
            Citizen.Wait(Config.DoorOpenRearRightTime)
            SetVehicleDoorOpen(GetVehiclePedIsIn(ped, false), 3, false, false)
        end
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            exports['progressBars']:startUI(Config.DoorBrokenRearRightTime, "Arka Sağ Kapı Parçalanıyor...")
	...
	if bonnet ~= -1 then
        ...
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            exports['progressBars']:startUI(Config.DoorBrokenHoodTime, "Kaput Parçalanıyor...")
	...
	if boot ~= -1 then
        ...
        Citizen.Wait(1000)
        if ChoppingInProgress == true then
            exports['progressBars']:startUI(Config.DoorBrokenTrunkTime, "Bagaj Parçalanıyor...")
            Citizen.Wait(Config.DoorBrokenTrunkTime)
            SetVehicleDoorBroken(GetVehiclePedIsIn(ped, false),5, true)
        end
    end
    Citizen.Wait(1000)
    exports['progressBars']:startUI(Config.DeletingVehicleTime, "İşlem Tamamlanıyor...")
...
AddEventHandler('lenzh_chopshop:hasExitedMarker', function(zone)
    ...
    if zone == 'Chopshop' then
        ...
        if ChoppingInProgress == true then
            exports.pNotify:SendNotification({text = "Alandan Çıktın ! Tekrar Alana Geri Dön !", type = "error", timeout = 1000, layout = "centerRight", queue = "right", killer = true, animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
        end
    end
    ChoppingInProgress        = false
